    ...
    <div className="w-full">
      <div className="flex justify-between">
        {steps.map((step, index) => (
          <div
            key={index}
            className={`flex flex-col items-center ${index <= currentStep ? "text-primary" : "text-foreground"}`}
          >
            <div className="relative">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center border-2 
                  ${
                    index < currentStep
                      ? "bg-primary text-primary-foreground border-indigo-200"
                      : index === currentStep
                        ? "border-indigo-600 text-primary"
                        : "border-indigo-200 text-muted-foreground"
                  }`}
              >
              ...
              {index < steps.length - 1 && (
                <div
                  className={`absolute top-1/2 left-full w-[calc(100%-2.5rem)] h-0.5 -translate-y-1/2 
                    ${index < currentStep ? "bg-primary" : "bg-foreground"}`}
